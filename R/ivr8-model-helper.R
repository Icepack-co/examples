...
make_distance_time_cap_dims <- function(){
  dimensions<-  new (IVR8.DimensionConfiguration)
  ...
  capdim <- new (IVR8.CapacityDimension)        # you can add as many capacity dimensions as you like in this model
...
make_job_time_cap <- function(jobdata, src, dest){
  ...
  for(i in 1:length(src)){
    ...
    a_time <- new (IVR8.Job.Task.Attribute)
    a_time$dimensionId <- 'time'
    a_time$quantity <- jobdata$pickupTime[di]

    a_cap <- new (IVR8.Job.Task.Attribute)
...
make_vcc_simple <- function(name, fixedcost, time_transit_costcoef,
  ...
  vcc_dist_cost <- new (IVR8.VehicleCostClass.Attribute)
...
make_vc_simple <- function(name, time_transit_coef, time_task_coef, time_loc_coef,
  ...
  vc <- new (IVR8.VehicleClass)
  vc$id <- name
  vca_t <- new (IVR8.VehicleClass.Attribute)
  ...
  vca_d <- new (IVR8.VehicleClass.Attribute)
...
make_vehicle_cap <- function(name,
  ...
  vcap <- new (IVR8.Vehicle.Capacity)
...
make_lunch_break_rule <- function(name, prefix, lunchtime, breaklength){
  ...
  trigger <-  new (IVR8.TransitRule.Trigger)
