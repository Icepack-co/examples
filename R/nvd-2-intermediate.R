...
periodLength <- 20
weekLength <- 5
repHomeLocations <- c('Person A', 'Person B')
data$id[9] <- repHomeLocations[1]
data$id[17] <- repHomeLocations[2]

m <- new (NVD.Model)
m$configuration <- new (NVD.Configuration)
m$configuration$timeCoef <- 1
m$configuration$timeCostCoef <- 0
m$configuration$distCostCoef <- 1
...
for(rhl in repHomeLocations){
  t <- new(NVD.Territory)
  t$id <- rhl
  t$dailyStartMinutes <- 0
  t$dailyEndMinutes <- 400
  t$location <- new (NVD.Geocode)
  t$location$longitude <- data %>% filter(id == rhl) %>% select(X) %>% unlist()
  t$location$latitude <- data %>% filter(id == rhl) %>% select(Y) %>% unlist()
  t$dailyStartMinutes <- rep(0, periodLength)
  t$dailyEndMinutes <- rep(440, periodLength)  # assuming a 6h40m hour work day, we don't want them overwhelmed
  # this can be modified quite easily on a day-to-day basis
  m$territories[[length(m$territories) + 1]] <- t
}

for(i in which(!(data$id %in% repHomeLocations))){
  v <- new (NVD.Visit)
  s<- data[i,]
  ...
  v$visitTimeMinutes <- s$visitTime
  v$territoryRelations <- new (NVD.Visit.TerritoryRelation)
  v$territoryRelations$type <- 0 # 0 INCLUSIVE, 1 EXCLUSIVE
  v$territoryRelations$territoryIds <- repHomeLocations #both reps by visit this location
...
instanceCheap <-  api %>% getSolutionInstance(sr, resp, solutionIndex = 1) # this is the cheapest solution
...
l$nodes %>%
  filter(!grepl(pattern = 'Person ', taskId, fixed =T)) %>%
  group_by(taskId, vehicleId) %>% summarise(numVisits = n(), visitDays = paste0(day, collapse = ','))
...
ggplot(data = l) +
  ...
  facet_grid(solution ~.) + ylab("Total Time (Minutes)")

#illustrates how you have many empty days as well as higher peaks (and generally a larger range)

l %>%
  group_by(solution) %>%
  summarise(total_minutes = sum(total_time_mins), total_distance = sum(distance_km), daily_variance = var(total_time_mins))
