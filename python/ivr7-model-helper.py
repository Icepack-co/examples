# a simple class which defines some common functions for working with the ivr7 series of models

def make_distance_time_cap_dims():
  dimensions = ivr7_kt461v8eoaif_pb2.DimensionConfiguration()
  ...
  capdim = ivr7_kt461v8eoaif_pb2.CapacityDimension()
  capdim.id = 'capacity'
  capdim.units = 'kg'
  capdim.slackMax = 0
  capdim.tardyMax = 0
...
def make_locations(data):
  ...
  for index, row in data.iterrows():
    l = ivr7_kt461v8eoaif_pb2.Location()
    l.id = row.id
...
def make_vcc_simple(name, fixedcost, time_transit_costcoef,
  ...
  vcc = ivr7_kt461v8eoaif_pb2.VehicleCostClass()
  vcc.id = name
  vcc.fixedCost = fixedcost # a fixed cost associated with using this particular vehicle
  vcc_time_cost = ivr7_kt461v8eoaif_pb2.VehicleCostClass.Attribute()
  ...
  vcc_dist_cost = ivr7_kt461v8eoaif_pb2.VehicleCostClass.Attribute()
...
def make_vc_simple(name, time_transit_coef, time_task_coef, time_loc_coef,
  ...
  vc = ivr7_kt461v8eoaif_pb2.VehicleClass()
  vc.id = name
  vca_t = ivr7_kt461v8eoaif_pb2.VehicleClass.Attribute()
  ...
  vca_d = ivr7_kt461v8eoaif_pb2.VehicleClass.Attribute()
...
def make_vehicle_cap(name,vehicleClass, vehicleCostClass, capacity,
                             startloc, endloc,
                             start_time, end_time):
  v = ivr7_kt461v8eoaif_pb2.Vehicle()
  v.id = name
  ...
  vatt = ivr7_kt461v8eoaif_pb2.Vehicle.Task.Attribute()
  ...
  vcap = ivr7_kt461v8eoaif_pb2.Vehicle.Capacity()
...
def make_lunch_break_rule(name, prefix, lunchtime, breaklength):
  tr = ivr7_kt461v8eoaif_pb2.TransitRule()
  tr.id = name
  tr.dimensionId = 'time'
  ...
  trigger =  ivr7_kt461v8eoaif_pb2.TransitRule.Trigger()
