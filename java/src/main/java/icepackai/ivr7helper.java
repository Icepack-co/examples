...
public class ivr7helper {
  public static void makeDistanceTimeCapDims(Ivr7Kt461V8Eoaif.Model.Builder m) {
    Ivr7Kt461V8Eoaif.DimensionConfiguration.Builder dimBuilder = Ivr7Kt461V8Eoaif.DimensionConfiguration.newBuilder();

    dimBuilder.setTimeConfig(Ivr7Kt461V8Eoaif.InternalDimension.newBuilder().setId("time")
        .setMeasurementUnit(eMeasurementUnit.MINUTES).setSlackMax(1e6f).setTardyMax(0).build());

    dimBuilder.setDistanceConfig(Ivr7Kt461V8Eoaif.InternalDimension.newBuilder().setId("distance")
        .setMeasurementUnit(eMeasurementUnit.KILOMETRES).setSlackMax(0).setTardyMax(0).build());

    dimBuilder.addCapacityDimensions(Ivr7Kt461V8Eoaif.CapacityDimension.newBuilder().setId("capacity").setUnits("kg")
        .setSlackMax(0).setTardyMax(0).build());

    m.setDimensions(dimBuilder.build());
  }

  public static void makeLocations(Ivr7Kt461V8Eoaif.Model.Builder m, List<dataRow> d, float windowStart,
      float windowEnd) {
    for (int i = 0; i < d.size(); i++) {
      dataRow loc = d.get(i);
      if (windowEnd == 0.0f && windowStart == 0.0f) {
        m.addLocations(Ivr7Kt461V8Eoaif.Location.newBuilder().setId(loc.id)
            .setGeocode(Ivr7Kt461V8Eoaif.Geocode.newBuilder().setLatitude(loc.Y).setLongitude(loc.X)).build());
      } else {
        m.addLocations(
            Ivr7Kt461V8Eoaif.Location.newBuilder().setId(loc.id)
                .setGeocode(Ivr7Kt461V8Eoaif.Geocode.newBuilder().setLatitude(loc.Y).setLongitude(loc.X))
  ...
  public static void makeJobTimeCap(Ivr7Kt461V8Eoaif.Model.Builder m, List<dataRow> d, List<Integer> srcs,
    ...
    for (int i = 0; i < srcs.size(); i++) {
      ...
      job.setPickupTask(
          Ivr7Kt461V8Eoaif.Job.Task.newBuilder().setTaskId("pickup_" + d.get(di).id).setLocationId(d.get(si).id)
              .addAttributes(Ivr7Kt461V8Eoaif.Job.Task.Attribute.newBuilder().setDimensionId("time")
                  .setQuantity(d.get(di).pickupTime).build())
              .addAttributes(Ivr7Kt461V8Eoaif.Job.Task.Attribute.newBuilder().setDimensionId("capacity")
                  .setQuantity(d.get(di).quantity).build())
              .build());

      job.setDropoffTask(
          Ivr7Kt461V8Eoaif.Job.Task.newBuilder().setTaskId("dropoff_" + d.get(di).id).setLocationId(d.get(di).id)
              .addAttributes(Ivr7Kt461V8Eoaif.Job.Task.Attribute.newBuilder().setDimensionId("time")
  ...
  public static Ivr7Kt461V8Eoaif.VehicleCostClass makeVccSimple(String name, float fixedcost,
    ...
    return Ivr7Kt461V8Eoaif.VehicleCostClass.newBuilder().setId(name).setFixedCost(fixedcost)
        .addAttributes(Ivr7Kt461V8Eoaif.VehicleCostClass.Attribute.newBuilder().setDimensionId("time")
        ...
        .addAttributes(Ivr7Kt461V8Eoaif.VehicleCostClass.Attribute.newBuilder().setDimensionId("distance")
            .setTransitCostCoef(distance_transit_costcoef).build())
        .build();
  }

  public static Ivr7Kt461V8Eoaif.VehicleClass makeVcSimple(String name, float time_transit_coef, float time_task_coef,
      float time_loc_coef, float distance_transit_coef) {
    return Ivr7Kt461V8Eoaif.VehicleClass.newBuilder().setId(name)
  ...
  public static Ivr7Kt461V8Eoaif.Vehicle makeVehicleCap(String name, String vehicleClass, String vehicleCostClass,
      float capacity, String startLocation, String endLocation, float startTime, float endTime) {
    return Ivr7Kt461V8Eoaif.Vehicle.newBuilder().setId(name).setClassId(vehicleClass).setCostClassId(vehicleCostClass)
        .setShift(Ivr7Kt461V8Eoaif.Vehicle.Shift.newBuilder().setShiftStart(Ivr7Kt461V8Eoaif.Vehicle.Task.newBuilder()
            .setLocationId(startLocation)
            .addAttributes(Ivr7Kt461V8Eoaif.Vehicle.Task.Attribute.newBuilder().setDimensionId("time").setQuantity(0.0f)
                .addWindows(Ivr7Kt461V8Eoaif.Window.newBuilder().setStart(startTime).setEnd(endTime)).build())
            .build())
            .setShiftEnd(Ivr7Kt461V8Eoaif.Vehicle.Task.newBuilder().setLocationId(endLocation)
                .addAttributes(Ivr7Kt461V8Eoaif.Vehicle.Task.Attribute.newBuilder().setDimensionId("time")
            ...
            .build())
        .addCapacities(
            Ivr7Kt461V8Eoaif.Vehicle.Capacity.newBuilder().setDimensionId("capacity").setCapacity(capacity).build())
        .build();
  }

  public static Ivr7Kt461V8Eoaif.TransitRule makeLunchBreakRule(String name, String prefix, float lunchtime,
      float breaklength) {
    return Ivr7Kt461V8Eoaif.TransitRule.newBuilder().setId(name).setDimensionId("time").setRuleIdPrefix(prefix)
        .setUseStandingState(true).setUseTransitState(true)
        .addTriggers(Ivr7Kt461V8Eoaif.TransitRule.Trigger.newBuilder().setValue(lunchtime).setQuantity(breaklength))
        .build();
  }
  ...
  public static void printSolution(Ivr7Kt461V8Eoaif.SolutionResponse solution, Boolean printnodes,
    ...
    if (hasTrRules && printTrrules) {
      ...
      List<Object[]> tabData = new ArrayList<Object[]>();
      for (Ivr7Kt461V8Eoaif.SolutionResponse.Route r : solution.getRoutesList()) {
        for (Ivr7Kt461V8Eoaif.SolutionResponse.TransitRuleAttribute tr : r.getTransitRuleAttributesList()) {
    ...
    if (printInfeas) {
      ...
      for (Ivr7Kt461V8Eoaif.SolutionResponse.Infeasibility i : solution.getInfeasibilitiesList()) {
        for (Ivr7Kt461V8Eoaif.SolutionResponse.Infeasibility.Info m : i.getInfeasibilityInfoList()) {
          tabData.add(
