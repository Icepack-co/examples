...
public class cvrptw1basic {
  ...
  public void Run() throws Exception {
    api = new apiHelper<CvrptwAcyas3Nzweqb.SolutionResponse>(CvrptwAcyas3Nzweqb.SolutionResponse.class, "cvrptw-acyas3nzweqb",
        configFile);
    ...
    CvrptwAcyas3Nzweqb.CVRPTW.Builder model = builder.getModel().toBuilder(); // this is the actual model container.
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      if (i == 0) { // treat the first point as the depot.
        model.setDepot(
            CvrptwAcyas3Nzweqb.Geocode.newBuilder().setId(row.id).setX(row.X).setY(row.Y).setQuantity(0.0f).build());
      } else {
        // we randomly allocate morning and afternoon time windows here.
        if( i % 2 == 0){
          model.addPoints(
            CvrptwAcyas3Nzweqb.Geocode.newBuilder()
            .setId(row.id)
            .setX(row.X)
            .setY(row.Y)
            .setWindowStart(8*60) // Morning window: 08:00 -> 12:00
            .setWindowEnd(12*60)
            .setQuantity(20.0f).build());
        }else{
          model.addPoints(
              CvrptwAcyas3Nzweqb.Geocode.newBuilder()
              .setId(row.id)
    ...
    CvrptwAcyas3Nzweqb.SolutionResponse solution = api.Get(requestId); // get the response (which is cast internally)
    ...
    String[] columnNames = { "Vehicle", "Stop", "Distance Travelled", "Cumulative Distance", "Cumulative load", "Arrival Time" };
    ...
    for (int i = 0; i < solution.getRoutesCount(); i++) {
      ...
      if(r.getVisitCapacitiesCount() > 0){
        ...
        tabData[totalStops] = new Object[] { "Route_" + i, r.getSequence(0), 0.0f, 0.0f, cumulCap, r.getArrivalTimes(0) };
        totalStops++;
        for (int j = 1; j < r.getSequenceCount(); j++) {
          ...
          tabData[totalStops] = new Object[] { "Route_" + i, r.getSequence(j), e.getDistance(), totalDistance, cumulCap, r.getArrivalTimes(j) };
          totalStops++;
