...
// * Illustrates how to use the parent solve-request container for evaluate requests against a
// versioned model
// ** This is a quicker manner in which to run evaluate requests against the api where a large chunk
//    of model content is required to define the model. Send it once, then just run requests against
//    that model, and we'll handle moving the data around on our side.
public class ivr7_4_advanced1 {
  ...
  public void Run() throws Exception {
    ...
    apiHelper<IvrdataO43E0Dvs78Zq.CachedModel> data_api =
        new apiHelper<IvrdataO43E0Dvs78Zq.CachedModel>(
            IvrdataO43E0Dvs78Zq.CachedModel.class, "ivrdata-o43e0dvs78zq", configFile);
    IvrdataO43E0Dvs78Zq.CachedModel dataModel =
        IvrdataO43E0Dvs78Zq.CachedModel.newBuilder().setModel(model.build().toByteString()).build();
    ...
    for (Ivr7Kt461V8Eoaif.SolutionResponse.Route r : Solution.getRoutesList()) {
      Ivr7Kt461V8Eoaif.TaskSequence.Builder nts =
          Ivr7Kt461V8Eoaif.TaskSequence.newBuilder().setVehicleId(r.getVehicleId());
      for (int i = 1; i < r.getStopsCount() - 1; i++) {
        nts.addTaskId(r.getStops(i).getTaskId());
      }
      builder.addRoutes(
          nts); // NOTE: We're adding the tasks to the solve request, not the model - the task
                // sequence will be applied to the model we've referenced in this example
    }
    builder.setSolveType(SolveType.Evaluate);
    System.out.println(builder.build().toString()); // so a very simple follow up request - but with
