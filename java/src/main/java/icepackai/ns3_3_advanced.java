...
public class ns3_3_advanced {
  public ns3_3_advanced(List<dataRow> inputdata, String config) throws Exception {
    ...
    this.data = inputdata.stream().limit(100).collect(Collectors.toList()); // grabs just the first 100 items.
  }
  ...
  public void Run() throws Exception {
    ...
    for(int i = 0; i < d_nodes.size(); i++){
      ...
      pf.setProductId("Beer")
      .addDimensionRanges(ns3helper.make_dimension_range("weight", demandNodes.get(i).quantity, demandNodes.get(i).quantity));
      ...
      d_nodes.set(i, d_nodes.get(i).toBuilder().addConsumption(pf.build()).addAllAllowableSources(allowableSources).build());
      // the nodes are built as compelete items here, so we can convert them, add to the node, and save it back in the list.
    }
    ...
    for(int i = 0; i < p_nodes.size(); i++){
      ...
      totalQty *= 0.9f; // note: we're decreasing the total amount the production node can manufacture here, so there will be demand that
                        // goes unserviced
      pf.setProductId("Beer")
      .addDimensionRanges(ns3helper.make_dimension_range("weight", 0, totalQty).toBuilder().setFlowPenalty(1e8f).build());
      // note the penalty on over-servicing on the production node is now greater than the demand nodes (i.e. it's a more strict soft-constraint)
    ...
    for(int i = 0; i < w_nodes.size(); i++){
      ...
      w_nodes.set(i, w_nodes.get(i).toBuilder().setFlow(
        Ns3Tbfvuwtge2Iq.Node.Flow.newBuilder().addFixedDimensionCosts(
          Ns3Tbfvuwtge2Iq.FixedDimensionCost.newBuilder()
    ...
    Ns3Tbfvuwtge2Iq.SolutionResponse solution = api.Get(requestId); // get the response (which is cast internally)
