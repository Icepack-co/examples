...
public class ns3_2_intermediate {
  public ns3_2_intermediate(List<dataRow> inputdata, String config) throws Exception {
  ...
  public void Run() throws Exception {
    ...
    for(int i = 0; i < d_nodes.size(); i++){
      ...
      d_nodes.set(i, d_nodes.get(i).toBuilder().addConsumption(pf.build()).addAllAllowableSources(sources).build());
    ...
    for(int i = 0; i < p_nodes.size(); i++){
      ...
      pf.setProductId("Beer")
      .addDimensionRanges(ns3helper.make_dimension_range("weight", 0, totalQty).toBuilder().setFlowPenalty(0.0f).build());
    ...
    // This is reflected in the node flow table - you'll notice the inflow and outflow for the Galway warehouse
    // is now zero.
