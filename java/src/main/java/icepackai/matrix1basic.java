...
public class matrix1basic {
  public matrix1basic(List<dataRow> inputdata, String config) throws Exception {
    ...
    this.data = inputdata.stream().limit(5).collect(Collectors.toList()); // grabs just the first 5 items.
  }

  public void Run() throws Exception {
    api = new apiHelper<MatrixVyv95N7Wchpl.MatrixResponse>(MatrixVyv95N7Wchpl.MatrixResponse.class,
        "matrix-vyv95n7wchpl", configFile);
    // so here we're going to build the model

    // create a solve request
    MatrixVyv95N7Wchpl.MatrixRequest.Builder builder = MatrixVyv95N7Wchpl.MatrixRequest.newBuilder();

    // add locations to the matrix request
    for (int i = 0; i < data.size(); i++) {
      dataRow row = data.get(i);
      builder.addLocations(MatrixVyv95N7Wchpl.Location.newBuilder().setId(row.id)
          .setGeocode(MatrixVyv95N7Wchpl.Geocode.newBuilder().setLatitude(row.Y).setLongitude(row.X).build()).build());
      builder.addSources(row.id); // this will ensure a complete matrix. because destinations are empty,
                                  // we assume you're asking for sources:sources
    }
    ...
    MatrixVyv95N7Wchpl.MatrixResponse solution = api.Get(requestId); // get the response (which is cast internally)
