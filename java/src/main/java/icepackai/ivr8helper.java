...
public class ivr8helper {
  public static void makeDistanceTimeCapDims(Ivr8Yni1C9K2Swof.Model.Builder m) {
    Ivr8Yni1C9K2Swof.DimensionConfiguration.Builder dimBuilder =
        Ivr8Yni1C9K2Swof.DimensionConfiguration.newBuilder();

    dimBuilder.setTimeConfig(Ivr8Yni1C9K2Swof.InternalDimension.newBuilder()
                                 .setId("time")
    ...
    dimBuilder.setDistanceConfig(Ivr8Yni1C9K2Swof.InternalDimension.newBuilder()
                                     .setId("distance")
    ...
    dimBuilder.addCapacityDimensions(Ivr8Yni1C9K2Swof.CapacityDimension.newBuilder()
                                         .setId("capacity")
                                         .setUnits("kg")
                                         .setSlackMax(0)
                                         .setTardyMax(0)
  ...
  public static void makeLocations(
      Ivr8Yni1C9K2Swof.Model.Builder m, List<dataRow> d, float windowStart, float windowEnd) {
    for (int i = 0; i < d.size(); i++) {
      dataRow loc = d.get(i);
      if (windowEnd == 0.0f && windowStart == 0.0f) {
        m.addLocations(
            Ivr8Yni1C9K2Swof.Location.newBuilder()
                .setId(loc.id)
                .setGeocode(
                    Ivr8Yni1C9K2Swof.Geocode.newBuilder().setLatitude(loc.Y).setLongitude(loc.X))
                .build());
      } else {
        m.addLocations(
            Ivr8Yni1C9K2Swof.Location.newBuilder()
                ...
                .setGeocode(
                    Ivr8Yni1C9K2Swof.Geocode.newBuilder().setLatitude(loc.Y).setLongitude(loc.X))
                .addAttributes(
                    Ivr8Yni1C9K2Swof.Location.Attribute.newBuilder()
                        .setDimensionId("time")
                        .addArrivalWindows(
                            Ivr8Yni1C9K2Swof.Window.newBuilder().setStart(8 * 60f).setEnd(
                                14 * 60f)))
                .build());
      }
    }
  ...
  public static void makeJobTimeCap(Ivr8Yni1C9K2Swof.Model.Builder m, List<dataRow> d,
    ...
    for (int i = 0; i < srcs.size(); i++) {
      ...
      job.setPickupTask(Ivr8Yni1C9K2Swof.Job.Task.newBuilder()
                            .setTaskId("pickup_" + d.get(di).id)
                            .setLocationId(d.get(si).id)
                            .addAttributes(Ivr8Yni1C9K2Swof.Job.Task.Attribute.newBuilder()
                                               .setDimensionId("time")
      ...
      job.setDropoffTask(Ivr8Yni1C9K2Swof.Job.Task.newBuilder()
  ...
  public static Ivr8Yni1C9K2Swof.VehicleCostClass makeVccSimple(String name, float fixedcost,
    ...
    return Ivr8Yni1C9K2Swof.VehicleCostClass.newBuilder()
        .setId(name)
        .setFixedCost(fixedcost)
        .addAttributes(Ivr8Yni1C9K2Swof.VehicleCostClass.Attribute.newBuilder()
                           .setDimensionId("time")
  ...
  public static Ivr8Yni1C9K2Swof.VehicleClass makeVcSimple(String name, float time_transit_coef,
      float time_task_coef, float time_loc_coef, float distance_transit_coef) {
    return Ivr8Yni1C9K2Swof.VehicleClass.newBuilder()
        .setId(name)
        .addAttributes(Ivr8Yni1C9K2Swof.VehicleClass.Attribute.newBuilder()
                           .setDimensionId("time")
  ...
  public static Ivr8Yni1C9K2Swof.Vehicle makeVehicleCap(String name, String vehicleClass,
      String vehicleCostClass, float capacity, String startLocation, String endLocation,
      float startTime, float endTime) {
    return Ivr8Yni1C9K2Swof.Vehicle.newBuilder()
        .setId(name)
        .setClassId(vehicleClass)
        .setCostClassId(vehicleCostClass)
        .setShift(
            Ivr8Yni1C9K2Swof.Vehicle.Shift.newBuilder()
                .setShiftStart(
                    Ivr8Yni1C9K2Swof.Vehicle.Task.newBuilder()
                        .setLocationId(startLocation)
                        .addAttributes(
                            Ivr8Yni1C9K2Swof.Vehicle.Task.Attribute.newBuilder()
                                .setDimensionId("time")
                                .setQuantity(0.0f)
                                .addWindows(
                                    Ivr8Yni1C9K2Swof.Window.newBuilder().setStart(startTime).setEnd(
                                        endTime))
                                .build())
                        .build())
                .setShiftEnd(Ivr8Yni1C9K2Swof.Vehicle.Task.newBuilder()
                                 .setLocationId(endLocation)
                                 .addAttributes(Ivr8Yni1C9K2Swof.Vehicle.Task.Attribute.newBuilder()
                                                    .setDimensionId("time")
                                                    // then just add the time windows to the shift
                                                    .setQuantity(0.0f)
                                                    .addWindows(Ivr8Yni1C9K2Swof.Window.newBuilder()
                                                                    .setStart(startTime)
                                                                    .setEnd(endTime))
                                                    .build())
                                 .build())
                .build())
        .addCapacities(Ivr8Yni1C9K2Swof.Vehicle.Capacity.newBuilder()
                           .setDimensionId("capacity")
  ...
  public static Ivr8Yni1C9K2Swof.TransitRule makeLunchBreakRule(
      String name, String prefix, float lunchtime, float breaklength) {
    return Ivr8Yni1C9K2Swof.TransitRule.newBuilder()
        .setId(name)
        .setDimensionId("time")
        ...
        .addTriggers(
            Ivr8Yni1C9K2Swof.TransitRule.Trigger.newBuilder().setValue(lunchtime).setQuantity(
                breaklength))
        .build();
  }
  ...
  public static void printSolution(Ivr8Yni1C9K2Swof.SolutionResponse solution, Boolean printnodes,
    ...
    if (printnodes) {
      String[] columnNames = {
          "stopId", "locationId", "taskId", "jobId", "vehicleId", "compartmentId"};
      ...
      for (int i = 0; i < solution.getRoutesCount(); i++) {
        ...
        if (r.getStopsCount() > 2) {
          for (int j = 1; j < r.getStopsCount() - 1; j++) {
            ...
            tabData.add(new Object[] {s.getId(), s.getLocationId(), s.getTaskId(), s.getJobId(),
                r.getVehicleId(), s.getCompartmentId()});
          }
    ...
    if (hasTrRules && printTrrules) {
      ...
      List<Object[]> tabData = new ArrayList<Object[]>();
      for (Ivr8Yni1C9K2Swof.SolutionResponse.Route r : solution.getRoutesList()) {
        for (Ivr8Yni1C9K2Swof.SolutionResponse.TransitRuleAttribute tr :
    ...
    if (printInfeas) {
      ...
      for (Ivr8Yni1C9K2Swof.SolutionResponse.Infeasibility i : solution.getInfeasibilitiesList()) {
        for (Ivr8Yni1C9K2Swof.SolutionResponse.Infeasibility.Info m :
  ...
  public static void printCompartmentSummary(
      Ivr8Yni1C9K2Swof.Model model, Ivr8Yni1C9K2Swof.SolutionResponse solution) {
    System.out.println("\nCompartment Summary:");
    ...
    String[] columnNames =
        new String[] {"locationId", "taskId", "jobId", "vehicleId", "compartmentId"};

    Map<String, Ivr8Yni1C9K2Swof.CompartmentSet> vehicleCset =
        new HashMap<String, Ivr8Yni1C9K2Swof.CompartmentSet>();
    ...
    Map<String, Ivr8Yni1C9K2Swof.Compartment> compartmentLookup =
        new HashMap<String, Ivr8Yni1C9K2Swof.Compartment>();
    ...
    for (Ivr8Yni1C9K2Swof.SolutionResponse.Route r : solution.getRoutesList()) {
      if (r.getStopsCount() > 2) {
        ...
        for (String c : cset.getCompartmentIdsList()) {
          for (Ivr8Yni1C9K2Swof.Compartment.Capacity capdim :
              compartmentLookup.get(c).getCapacitiesList()) {
        ...
        for (String dim : activeCaps) {
          ...
          toprow[0] = "capacity";
          for (int i = 0; i < nc; i++) {
            for (Ivr8Yni1C9K2Swof.Compartment.Capacity cap :
                compartmentLookup.get(cset.getCompartmentIds(i)).getCapacitiesList()) {
          ...
          for (int si = 1; si < r.getStopsCount() - 1;
               si++) { // skip the first and last stop because we know there is
                       // no compartment interaction there.
            Ivr8Yni1C9K2Swof.SolutionResponse.Stop s = r.getStops(si);
            if (s.getCompartmentId() != "") {
              for (Ivr8Yni1C9K2Swof.SolutionResponse.StopAttribute a : s.getAttributesList()) {
                if (dim.compareTo(a.getDimId())
                    == 0) { // so java is a bit sticky about string comparisons
                            // this is the bullet proof way to compare apparently
                  float nv = compartmentValues.get(s.getCompartmentId())
                      + (a.getEndValue() - a.getStartValue());
