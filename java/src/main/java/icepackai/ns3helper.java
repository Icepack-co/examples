...
public class ns3helper {
  ...
  public static Ns3Tbfvuwtge2Iq.DimensionRange make_dimension_range(String dimid, float min, float max){
  ...
  public static Ns3Tbfvuwtge2Iq.UnitDimensionCost make_udc(String dimid, float coef, float costperunit){
    Ns3Tbfvuwtge2Iq.UnitDimensionCost.Builder udc = Ns3Tbfvuwtge2Iq.UnitDimensionCost.newBuilder();
    return udc.addDimensionIds(dimid).addDimensionCoefficients(coef).setCostPerUnit(costperunit).build();
  }

  public static Ns3Tbfvuwtge2Iq.LaneRate make_lane_rate_distance(String src, String dest, float costperkm ){
    Ns3Tbfvuwtge2Iq.LaneRate.Builder lr  = Ns3Tbfvuwtge2Iq.LaneRate.newBuilder();
    return lr.setId( "lr:" + src + "->" + dest)
    .setSource(src)
    .setDestination(dest)
    .addUnitDimensionCosts(make_udc("distance", 1.0f, costperkm)).build();
  }

  public static Ns3Tbfvuwtge2Iq.LaneRate make_lane_rate_distance_weight(String src, String dest, float costperkm, String weightdim, float costperunit){
    Ns3Tbfvuwtge2Iq.LaneRate.Builder lr  = Ns3Tbfvuwtge2Iq.LaneRate.newBuilder();
  ...
  public static Ns3Tbfvuwtge2Iq.CostModel make_cost_model_distance(String src, float costperkm){
    ...
    return cm.setId("costmodel: " + src + ":Beer").setSource(src).addProductGroupIds("Beer").addUnitDimensionCost(make_udc("distance", 1.0f, costperkm)).build();
  }

  public static void printSolution(Ns3Tbfvuwtge2Iq.SolutionResponse solution){
    
    System.out.println("Assignment table:");
    String[] columnNames = new String[] { "Lane Rate", "Cost Model", "Source","Destination", "Product", "Amount", "Cost", "Distance", "Duration"};
    List<Object[]> tabData = new ArrayList<Object[]>();
    
    Integer maxPrint = 0;
    for (Ns3Tbfvuwtge2Iq.SolutionResponse.Assignment r : solution.getAssignmentsList()) {
      tabData.add(new Object[] { r.getLaneRateId(), r.getCostModelId(), r.getSource(), r.getDestination(),
    ...
    if(maxPrint > 10){
      System.out.println("Ouputput truncated..." + (solution.getAssignmentsCount() - maxPrint) + " items skipped.");
    }
    ...
    for(Ns3Tbfvuwtge2Iq.SolutionResponse.NodeFlow nf : solution.getNodeFlowsList()){
      tabData.add(new Object[] { nf.getNodeId(), nf.getInFlow(), nf.getOutFlow(), nf.getFixedCost(),
        nf.getFlowCost(), nf.getProductFixedCost(), nf.getProductFlowCost(), nf.getProductionAmount(), nf.getProductionCost(),
      nf.getProductionPenalty(), nf.getConsumptionAmount(), nf.getConsumptionCost(), nf.getConsumptionPenalty() });
      maxPrint++;
      if(maxPrint > 10){
        break;
      }
    ...
    for(Ns3Tbfvuwtge2Iq.SolutionResponse.NodeProductFlow nf : solution.getNodeProductFlowsList()){
      tabData.add(new Object[] { nf.getNodeId(), nf.getInFlow(), nf.getProductId(), nf.getOutFlow(), nf.getFixedCost(),
        nf.getFlowCost(),  nf.getProductionAmount(), nf.getProductionCost(),
      nf.getProductionPenalty(), nf.getConsumptionAmount(), nf.getConsumptionCost(), nf.getConsumptionPenalty() });
      maxPrint++;
      if(maxPrint > 10){
        break;
    ...
    // then just for the curious reader. Each of the geometries is available in the payload. The route defines the 
    // sequence of geometry segments that should be joined to form the linestring for a particular route.
